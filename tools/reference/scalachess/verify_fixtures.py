...
def verify_batch(
    ...
    for index, (fixture, observed) in enumerate(zip(fixtures, actual, strict=True)):
        require(
            observed["fixture_index"] == str(index + index_offset),
            f"{fixture['id']}: output order mismatch",
        )
...
def main() -> int:
    ...
    encoded_histories = [
        ...
        for fixture in material_histories
    ] + [
        encode("\n".join([fixture["initial_fen"], *fixture["moves"]]))
    ...
    require(
        len(batches["histories"])
        == len(histories) + len(material_histories) + len(repetition_histories),
    ...
    material_start = len(histories)
    repetition_start = material_start + len(material_histories)
    verify_batch(
        material_histories,
        batches["histories"][material_start:repetition_start],
        "material histories",
        material_start,
    )
    ...
    for fixture, observed in zip(
        ...
        batches["histories"][repetition_start:],
        strict=True,
    ):
